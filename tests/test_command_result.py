...
async def _run_send_command(response: bytes | None) -> tuple[bool, AsyncMock]:
    ...
    with (
        ...
        result = await client.async_send_command(CMD_SET_POWER_MODE, [1, 0], ALIAS_W5)

    return result, disconnect


@pytest.mark.asyncio
async def test_command_timeout_returns_false() -> None:
    """A command with no matching response must not be reported as successful."""
    result, disconnect = await _run_send_command(None)
...
@pytest.mark.asyncio
async def test_command_response_returns_true() -> None:
    """A received response continues to report command success."""
    result, disconnect = await _run_send_command(b"\x01")
...
@pytest.mark.asyncio
async def test_empty_response_payload_is_still_a_response() -> None:
    """An empty payload is distinct from a timeout and remains successful."""
    result, disconnect = await _run_send_command(b"")
